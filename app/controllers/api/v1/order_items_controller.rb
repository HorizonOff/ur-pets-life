...
class OrderItemsController < Api::BaseController
  ...
  def get_pending_order_items
    pending_orders = OrderItem.includes(:order).where(order_items: {status: [:pending, :confirmed, :on_the_way]}, orders: { user_id: @user.id, Payment_Status: 1 })
  ...
  def get_completed_order_items
    completed_orders = OrderItem.includes(:order).where(order_items: {status: [:delivered, :delivered_by_card, :delivered_by_cash, :delivered_online, :cancelled]}, orders: {user_id: @user.id})
